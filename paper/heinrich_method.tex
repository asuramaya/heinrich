...
\emph{Cliff falsification.} The three-quarters-depth commit of
Section~\ref{sec:worked} cannot occur in these models: a causal bank has no depth
for a seam to live in. Running the depth-structure analysis on one anyway returns
one layer and an empty list of per-layer structure---the frame declines to find a
cliff where none can exist. The commit band is therefore a property of the models,
not a shape the method paints onto everything it touches.
